...
export default function Listing(props) {
    const deleteListing = listing => {
        const confirmDelete = window.confirm(`Are you sure you want to delete the listing for ${listing.firstName} ${listing.lastName}?`);
        if (!confirmDelete) {
            return;            
        }
    ...
    return <tr> 
        <td>
            (<button className="btn" onClick={() => deleteListing(props.listing)}>Del</button>)  
            (<Link className="btn" to={{pathname:"/listing", state: {listingToUpdate: props.listing}}}>Edit</Link>)
        </td>     
